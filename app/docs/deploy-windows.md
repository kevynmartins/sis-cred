...
Carregue o schema (via PowerShell, na pasta do projeto):

```powershell
Get-Content .\database\schema.sql -Raw | & "C:\Program Files\MariaDB 11.x\bin\mariadb.exe" -u root -p
```

## 3. Copiar o código e instalar dependências

```powershell
New-Item -ItemType Directory -Force -Path C:\sis-cred
# copie o conteúdo da pasta app/ para C:\sis-cred
Set-Location C:\sis-cred
npm ci
```
...
notepad .env   # preencha DB_PASSWORD, JWT_SECRET, CORS_ORIGIN, SMTP_*
```
...
em um PowerShell aberto **dentro** de `C:\sis-cred`, ajustando o caminho do `nssm.exe`
se necessário):
...
- Redireciona stdout/stderr para `C:\sis-cred\logs\`.
- Inicia o serviço.
...
1. No **Gerenciador do IIS**, crie um novo site:
   - Nome: `Sis-Cred`
   - Caminho físico: `C:\sis-cred\dist`
   - Binding: porta 80 (e 443 depois de instalar o certificado).
2. Copie `deploy\windows\web.config` para dentro de `C:\sis-cred\dist\web.config`
...
4. Reinicie o site (`iisreset` ou pelo Gerenciador do IIS).
...
```powershell
Set-Location C:\sis-cred
Stop-Service SisCredApi
# copie os arquivos atualizados / git pull
npm ci
npm run build:prod
# aplique novas migrations em database\migrations\, se houver
Start-Service SisCredApi
iisreset
```
